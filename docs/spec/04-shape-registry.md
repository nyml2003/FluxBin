...
当 frame 被判定为 `typed` 模式后，Registry 才参与：
...
当前方向不应再只围绕“对象字段列表”建模。

更合理的方向是统一 `CompiledNode` 树，再把 object 视为其中一种 node。
...
type CompiledObjectArrayNode = {
  kind: "object-array";
  item: CompiledObjectNode;
  fixedWidth: false;
  staticByteLength: null;
  depth: number;
};

type CompiledScalarArrayNode = {
  kind: "scalar-array";
  item: PrimitiveNode;
  fixedWidth: false;
  staticByteLength: null;
...
- `u32 + bool + i16` 是定长
- `u32 + utf8-string` 是变长
- `objectArray` / `scalarArray` 必定视为变长
- `variant` 默认视为变长
...
当 node 树里含变长节点时，后部字段偏移通常不能完全静态确定。
