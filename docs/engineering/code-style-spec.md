...
当前目标不是彻底禁用所有可选语法，而是尽量减少它们在主实现里的使用。
...
- 每个新增或重写的运行时代码文件，文件开头默认使用 `/** ... */` 块注释写整体概述
- 概述至少说明：
  - 文件负责什么
  - 它属于哪一层
  - 它不负责什么
...
```ts
/**
 * 这里先记录 pending request，再发 frame。
...
- 不只看全局 coverage
- 纳入统计的单文件 coverage 目标也要 >= 80%

这条要求应在最终硬化阶段执行，不要求每个临时 skeleton 立即满足。
